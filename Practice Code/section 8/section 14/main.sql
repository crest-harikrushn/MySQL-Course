...
-- ? CH2
SELECT title, avg(rating) as avg_rating from series inner join reviews on series.id = reviews.series_id group by title;

-- ? CH3
 SELECT title, round(avg(rating), 2) as avg_rating from series inner join reviews on series.id = reviews.series_id group by title order by avg_rating; 

-- * without rounding off value of avg
 SELECT title, avg(rating) as avg_rating from series inner join reviews on series.id = reviews.series_id group by title order by avg_rating; 


-- ? CH4
SELECT first_name, last_name, rating from reviewers inner join reviews on reviewers.id = reviews.reviewer_id; 
SELECT first_name, last_name, rating from reviewers inner join reviews on reviewers.id = reviews.reviewer_id limit 15; 
